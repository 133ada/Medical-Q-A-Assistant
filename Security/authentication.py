"""HTTP authentication resolution for canonical Routers."""
...
def resolve_user_id(request: Request) -> Optional[str]:
    """Resolve an optional bearer identity without trusting request payloads."""
    ...
    user_id = next(
        (
            mapped_user
            for configured_token, mapped_user in token_map.items()
