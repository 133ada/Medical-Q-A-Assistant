"""
DrugBank OtherInformations Markdown → 本地向量库（增量构建版）
=============================================
只嵌入除 Drug Interactions 外的其他药物信息文件，来源目录：
  E:\\DrugBank_Details\\drugbank_md\\OtherInfoamations

逻辑与原 vector_store.py 完全一致，仅修改：
  - MD_DIR         → 指向 OtherInformations 专用目录
  - CHROMA_DIR     → 独立的 Chroma 持久化目录（与 ddi 分开存储）
  - HASH_FILE      → 独立的指纹文件
...
MD_DIR = r"E:\DrugBank_Details\drugbank_md\OtherInfoamations"
CHROMA_DIR = r"E:\PycharmProjects\Knowledge Q&A Assistant\app\rag\chroma_drugbank_other"
HASH_FILE = r"E:\PycharmProjects\Knowledge Q&A Assistant\app\rag\chroma_drugbank_other\doc_hashes.json"
...
def load_doc(source_key: str, filepath: Path) -> Document:
    ...
    # source_key 形如 "DB00001_Lepirudin_otherInformations"
...
def build_vectorstore(embeddings, MAX_FILES: Optional[int] = 0) -> Chroma:
    ...
    print(f"📁 [Other] 扫描到 {len(file_map):,} 个 .md 文件")
    ...
    print(f"📂 [Other] {'加载已有' if existing_count > 0 else '初始化新'}向量库"
    ...
    if this_batch:
        ...
        if not deleted_keys and not added_keys:
            print("✅ [Other] 所有文件均无变化，直接使用缓存向量库")
    ...
    if remaining > 0:
        print(f"\n💡 [Other] 提示：还剩 {remaining:,} 个文件未嵌入，将 MAX_FILES 调大后再次运行。")

    final_count = vectorstore._collection.count()
    print(f"\n✅ [Other] 向量库就绪，共 {final_count:,} 个向量块  |  目录: {CHROMA_DIR}\n")
    return vectorstore
